...
|`pipeline complete` | `{datasets:[{fileId, filters:[{id, label}], indicators:[{id, label}], timePeriod, geographicLevels, relevanceReason, autoSelectedFilterItems:{<filterLabel>:{filterItemLabel, filterItemId}}, unfilteredFilters:[<filterLabel>], isValidForTableGeneration, validationErrors:[{code, message}], validationWarnings:[{code, message}]}], token_usage, cost}` |
